...
{ 
	...
	{
		std::cout << "Feature module == SHOT" << std::endl;
  ...
  Result *result, pcl::Correspondences &inliers, std::vector<int> &LF_points_counter, std::vector<int> &LF_operations_counter)
{
	...
	if (g_ICP_SOLVER == "LM")
	{
		...
		std::cout << "ICP Iteration Number: " << icp_lm.getIterNumber() << std::endl;
		std::cout << "ICP Fitness Score: " << icp_lm.getFitnessScore() << std::endl;
	}
}
