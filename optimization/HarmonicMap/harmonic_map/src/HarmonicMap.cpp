...
void MeshLib::CHarmonicMap::map()
{
    ...
    for (M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
    {
        ...
        for (M::VertexVertexIterator witer(pV); !witer.end(); ++witer)
        {
            ...
            sw += w;
            Eigen::Triplet<double> elem;
            if (pW->boundary())
            {
                elem = Eigen::Triplet<double>(vid, wid, w);
                B_coefficients.push_back(elem);
            }
            else
            {
                elem = Eigen::Triplet<double>(vid, wid, -w);
                A_coefficients.push_back(elem);
            }
        }
        // insert the diagonal element
        Eigen::Triplet<double> diagElem = Eigen::Triplet<double>(vid, vid, sw);
        A_coefficients.push_back(diagElem);
    }
...
void MeshLib::CHarmonicMap::_calculate_edge_weight()
{
    ...
    for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); ++fiter)
    {
        ...
        M::CEdge *pE[3];
        pH[0] = (M::CHalfEdge *)pF->halfedge();
        pH[1] = (M::CHalfEdge *)pH[0]->he_next();
        pH[2] = (M::CHalfEdge *)pH[1]->he_next();

        pE[0] = (M::CEdge *)pH[0]->edge();
        pE[1] = (M::CEdge *)pH[1]->edge();
        pE[2] = (M::CEdge *)pH[2]->edge();

        pH[0]->angle() = _inverse_cosine_law(pE[1]->length(), pE[2]->length(), pE[0]->length());
        pH[1]->angle() = _inverse_cosine_law(pE[2]->length(), pE[0]->length(), pE[1]->length());
        pH[2]->angle() = _inverse_cosine_law(pE[0]->length(), pE[1]->length(), pE[2]->length());
    }
    ...
    for (M::MeshEdgeIterator eiter(m_pMesh); !eiter.end(); ++eiter)
    {
        ...
        if (pE->boundary())
        {
            M::CHalfEdge *he = (M::CHalfEdge *)pE->halfedge(0);
            pE->weight() = cos(he->angle()) / sin(he->angle());
        }
        else
        {
            M::CHalfEdge *he = (M::CHalfEdge *)pE->halfedge(0);
            double mid_calc = cos(he->angle()) / sin(he->angle());
            he = (M::CHalfEdge *)pE->halfedge(1);
            pE->weight() = mid_calc + (cos(he->angle()) / sin(he->angle()));
        }
        // std::cout << "pE Weight =" << pE->weight() << "\n";
    }
}
